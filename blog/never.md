...
在这里，`[[noreturn]]` 用来标注函数永远不会返回。你可能已经发觉，`[[noreturn]] void` 这两个在一起似乎有些多余。难道不会返回的函数还能声明为非 `void` 的情况吗？这个问题我们稍后给出答案，但肯定的是这样写确实有些啰嗦。所以许多现代语言引入了 `never` 类型（一些语言称为 `nothing`），专门用于表示这种不会返回的函数类型。

可以注意到不仅是声明为 `[[noreturn]]` 的函数有不返回的特点，`break` `return` `throw` 这样的控制语句在构成表达式的时候也具有这样的性质。
...
```
let value = a != 0 ? a : throw Exception();
```

显然两个分支的类型分别是 `A` 和 `never`。由于不可能经过第二个分支而返回值，所以 `value` 的类型可以直接取 `A`。即 `A | never = A`。这个恒等式实际上就说明 $\text{never} \subset A$。

稍加思索你会发现，对于任何类型，这个结果都满足。换句话，任何类型都是 `never` 的父类型。这也与实践相吻合：既然不会返回，那么实际上可以 `never` 当做任意类型的值来看待。特别的，由于 `never` 实际上不可能有任何的取值，我们可以得出 $\text{never} = \varnothing$，这也符合上面的数学逻辑。
...
所谓 `Option<T>`，即 `T | ()`，要么有值，要么是 `()`。这样在使用的时候就必须先排除是 `()` 的情形。

另一种方法称为 Nullability，即默认普通的类型 `T` 不能包含 `null`，只有显式添加一个问号 `T?` 的时候，才可以是接受 `null`。显然 `T? = T | null`，是 `T` 的父类型，不能直接转换为 `T`，进而就需要额外的检查才能使用。

前一种方案往往用于没有 `null` 的情况。而我们如果要讨论 `never` 与 `null` 并存，后一种方案更为合理。

### 结论

答案已经呼之欲出，根据 `T? = T | null`：

令 $T = \varnothing$，那么 $ T? = \varnothing? = \varnothing \cup \text{null} = \text{null}$。

或写作：

令 `T = never`，那么 `T? = never? = never | null = null`。
